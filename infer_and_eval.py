...
def load_label_maps(model_dir: str):
    ...
    if not os.path.isfile(path):
        raise FileNotFoundError(f"Не найден {path}. Убедись, что используешь директорию best чекпойнта.")
    with open(path, "r", encoding="utf-8") as f:
        maps = json.load(f)
    # привести ключи id2label к int
    id2label = {int(k): v for k, v in maps["id2label"].items()}
    label2id = {k: int(v) for k, v in maps["label2id"].items()}
    return label2id, id2label

@torch.no_grad()
def predict_df(model_dir: str,
               df: pd.DataFrame,
               text_col: str,
               max_length: int = 256,
               batch_size: int = 64):
    ...
    dt = time.time() - t0
    probs = np.vstack(all_probs) if all_probs else np.zeros((0, model.config.num_labels), dtype=np.float32)
    avg = dt / max(len(texts), 1)
    return np.array(preds, dtype=int), probs, avg

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--model_dir", required=True, help="Путь к директории best чекпойнта (где label_maps.json)")
    ...
    # 2) карты меток из чекпойнта
    label2id, id2label = load_label_maps(args.model_dir)
    ...
    if label_col is not None:
        ...
        if n_ok == 0:
            print("[metrics] Внимание: ни одна истинная метка не совпадает с label_maps.json — метрики не посчитаны.")
            return
